...
def pws(string:str, hints:str = "none", sub_function: bool = False):
    if LOG is False:
        return
...
def fileio(location: str, mode: str, data: str = ""):
    ...
    pws(f"Working with file '{location}', with mode '{mode}'.")
...
def css_format(html_string: str):
    ...
    pws("Finished replacing CSS ID",sub_function=True)
    ...
    pws("Finished replacing CSS Class",sub_function=True)
    
    pws(f"Success formatting for id '{random_hex}'")
...
def load_components_data():
    ...
    for dirpath, _, filenames in os.walk(VAR.components_dir):
        for file in filenames:
            if file.endswith(VAR.components_suffix):
                ...
                pws(f"Found component '{file.removesuffix(VAR.components_suffix)}'",sub_function=True)
    ...
    pws("Done loading components's HTML and CSS.")
    return
...
def init_build_dir():
    ...
    pws("Removed old directory.",sub_function=True)
    shutil.copytree(VAR.frontend, VAR.frontendbuild, dirs_exist_ok=True)
    pws("Copied build directory.",sub_function=True)
    pws("Initialized build directory.")
